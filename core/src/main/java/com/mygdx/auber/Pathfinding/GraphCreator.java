...
public class GraphCreator extends ApplicationAdapter {
    ...
    /**
     * Renders the nodes and paths on the screen using their render methods, used
    ...
    /**
     * Generates a node on every tile in the map, if it matches the id Called when
     * the GraphCreator object is instantiated
     */
    public void generateNodeMap() {
        for (int i = 0; i < tileLayer.getWidth(); i++) {
            for (int j = 0; j < tileLayer.getHeight(); j++) // Scans every tile
            {
                ...
                if (cell != null && cell.getTile() != null && (cell.getTile().getProperties().containsKey("node"))) // If
                                                                                                                    // ID
                                                                                                                    // matches
                                                                                                                    // floor/corridor
                                                                                                                    // tiles,
     ...
     * right) Called when GraphCreator is instantiated, after the nodes are created
     */
