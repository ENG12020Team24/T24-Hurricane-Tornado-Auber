...
public class KeySystemManager {
    ...
    private static Array<KeySystem> keySystems = new Array<>();
    /** The tile layer containing key systems managed by this object. */
    private final TiledMapTileLayer keySystemLayer;

    /** Class constructor.
     * @param tileLayer The tile layer to check for key systems on.
     */
    public KeySystemManager(final TiledMapTileLayer tileLayer) {
        this.keySystemLayer = tileLayer;
        loadKeySystems(tileLayer);
    }

    /**
     * Scans the tile layer and adds a KeySystem to keySystens upon finding a
     * tile with the property "keysystem".
     * @param tileLayer layer to scan for key systems
     */
    private static void loadKeySystems(final TiledMapTileLayer tileLayer) {
        for (int i = 0; i < tileLayer.getWidth(); i++) {
            ...
            for (int j = 0; j < tileLayer.getHeight(); j++) {
                ...
                if (cell != null && cell.getTile() != null && cell.getTile()
                    .getProperties().containsKey("keysystem")) {
                    // If ID matches floor/corridor tiles, and is not null
                    // System.out.print("i: ");
                    // System.out.print(i);
                    // System.out.print(", j: ");
                    // System.out.println(j);
                    String name = (String) cell.getTile()
    ...
    /**
     * @return The number of key systems that have both not been destroyed and
     ...
     */
    public static KeySystem getClosestKeySystem(
        ...
        for (KeySystem keySystem : keySystems) {
            ...
            if (keySystem.getPosition().dst(x, y)
                < closest.getPosition().dst(x, y)) {
                closest = keySystem;
            }
        }
        return closest;
    ...
    /**
     * @return The number of key systems, no matter if they have been destroyed
     * or not.
     */
     ...
     * @return The list of key systems managed by this object.
     */
    public static Array<KeySystem> getKeySystems() {
        return keySystems;
    }
}
